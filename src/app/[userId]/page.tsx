...
export default function Profile({ params }: { params: { userId: string } }) {
    ...
    const userId = api.profile.getProfile.useSuspenseQuery(params.userId);
    const postNumber = api.profile.getPostCount.useSuspenseQuery(params.userId)[1].data;
    const commentNumber = api.profile.getCommentCount.useSuspenseQuery(params.userId)[1].data;
    const posts = api.profile.getAllPostsFromUser.useSuspenseQuery(params.userId)[1].data;
                    ...
                    <div className="mt-2 bg-white rounded-lg">
                        <div className="relative mb-4 -mt-4">
                            <Suspense fallback={<Loading></Loading>}>
                                {(userId[1].data?.image) ? <div className="absolute left-4 -top-5 md:w-32 md:h-32 md:left-1/2 md:-top-10 md:-translate-x-[50%] rounded-full bg-transparent">
                                    <Image src={userId[1].data?.image ?? ""} alt="" width={1000} height={1000} className="w-28 h-auto rounded-full" />
                                </div> : null}
                            </Suspense>
                            <div className="flex justify-end pt-6 pr-6 md:top-16">
                                {userId[1].data?.id == session?.user.id ?
                                ...
                                <button className="px-2 hover:bg-[rgb(0,0,0)]/5 rounded-lg">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" role="img" aria-labelledby="abyohdzalx6c4nxnqym92x9n2cecu84w" className="crayons-icon dropdown-icon"><title id="abyohdzalx6c4nxnqym92x9n2cecu84w">User actions</title><path fill-rule="evenodd" clipRule="evenodd" d="M7 12a2 2 0 11-4 0 2 2 0 014 0zm7 0a2 2 0 11-4 0 2 2 0 014 0zm5 2a2 2 0 100-4 2 2 0 000 4z"></path></svg>
                                </button>
                            </div>
                        </div>
                        <div className="p-3 md:p-4">

                            <h1 className="mb-2 md:text-center font-bold text-3xl">{userId[1].data?.name}</h1>
                            <h1 className="mx-auto md:text-center md:text-lg mb-4 ">{(userId[1].data?.bio ? userId[1].data?.bio : "No bio for this user")}</h1>
                        </div>
                    </div>
                </div>
                <div className="pt-4 md:pt-0 md:grid md:grid-cols-[2fr_5fr] md:gap-4 w-100 max-w-[1024px] md:mx-2 rounded-lg lg:mx-auto">
                    <div className="hidden md:block h-fit p-4 bg-white rounded-lg">
                        ...
                        <div className="flex items-center mb-4  text-base">
                            ...
                            <Suspense fallback={<Loading />}>
                                {commentNumber} </Suspense> comments written
                        </div>
                            ...
                            <Suspense fallback={<Loading></Loading>}>
                                {posts.map((post, index) => {
                                    return <div key={index} className="mb-4">
                                        <Post post={post} user={{
                                            name: userId[1].data?.name as string,
                                            image: userId[1].data?.image as string,
                                            id: userId[1].data?.id as string
                                        }} />
                                    </div>;
                                })}
                            </Suspense>
